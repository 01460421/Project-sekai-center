/* 行事曆訂閱源：GET /cal/sekai.ics（手機用 webcal://games.project-sekai-center.com/cal/sekai.ics 訂閱）。
   從台服 master 的 events.json／gachas.json 產 iCalendar：近 60 天到未來的活動與卡池，
...
export function buildIcs(events, gachas, now) {
...
export async function handleCal(req, env, url) {
  ...
  let body;
  try { const [ev, ga] = await Promise.all([getJson('events.json'), getJson('gachas.json')]); body = buildIcs(ev, ga, Date.now()); }
