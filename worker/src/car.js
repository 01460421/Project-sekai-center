...
function confirmText(purpose, cc, extra) {
  ...
  if (purpose === 'register') {
    return '【注意】你正在用这个 QQ 注册一个新的网页账号，注册后该账号会绑定这个 QQ（可用来进入车队页、重置密码）。'
      + '\n只有你本人刚在网页上点了「用 QQ 注册」才继续；如果是别人叫你发的，请忽略，不要发送下面的确认码。' + tail;
  }
  if (purpose === 'link') {
    return '【注意】你正在把这个 QQ 绑定到网页账号「' + (extra || '未命名') + '」。绑定后该网页账号可以用你的 QQ 身分进入车队，也能用这个 QQ 重置密码。'
      + '\n只有那是你本人的网页账号才继续；如果是别人叫你发的，请忽略，不要发送下面的确认码。' + tail;
  }
  return '【注意】你正在重置网页账号「' + (extra || '') + '」的密码。'
