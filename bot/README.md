...
| 72 | `/trade` | 把你的重複卡送給別人（對方要按接受） | 按鈕 |
| 73 | `/wishlist` | 願望單與推し：設定最想抽到的角色，抽到 4★ 時會特別提示 | 自動完成 |
| 74 | `/gachalist` | 台服卡池情報：進行中與兩週內的卡池 | — |

### 🎵 問答與音樂（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 75 | `/quiz` | 綜合問答：世界計畫題或常識題，答對得水晶 | 按鈕 |
| 76 | `/songquiz` | 猜歌：從 BPM、長度、難度等提示猜出是哪一首 | 按鈕 |
| 77 | `/charaquiz` | 猜角色：從生日、團體、暱稱猜出是誰 | 按鈕 |
| 78 | `/speedquiz` | 搶答：全頻道一起搶，最先答對的人得分 | 按鈕 |
| 79 | `/quizrank` | 問答排行榜（本伺服器） | — |
| 80 | `/song` | 歌曲查詢：難度、物量、BPM、長度 | 自動完成 |
| 81 | `/randomsong` | 隨機選一首歌（可指定難度與等級範圍） | 按鈕 |
| 82 | `/setlist` | 隨機歌單：協力前抽 N 首歌 | — |
| 83 | `/songbattle` | 歌曲對決：兩首歌，大家投票選出比較喜歡的 | 按鈕 |
| 84 | `/event` | 台服活動與卡池日曆：進行中與即將開始 | — |

### 🏆 等級與社群（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 85 | `/rank` | 我的等級與經驗（聊天就會累積） | 被動事件 |
| 86 | `/leaderboard` | 排行榜（本伺服器）：等級、發言、聲望、遊戲勝場、本週發言、歐洲人（4★ 率） | — |
| 87 | `/achievements` | 成就一覽：已解鎖與未解鎖 | — |
| 88 | `/quest` | 每日任務：完成後領取獎勵 | 按鈕 |
| 89 | `/streak` | 連續簽到與活躍統計 | — |
| 90 | `/raffle` | 抽獎：大家按按鈕參加，發起人或時間到自動開獎 | 按鈕、排程 |
| 91 | `/team` | 隨機分組：把名單（或你所在語音頻道的人）分成 N 組 | — |
| 92 | `/remind` | 設定提醒：幾分鐘後在這個頻道 @ 你 | 排程 |
| 93 | `/countdown` | 倒數計時：顯示動態倒數，時間到會通知 | 排程 |
| 94 | `/counter` | 共用計數器：例如記今天打了幾場協力，大家都能按 +1 | 按鈕 |

### ⚙️ 伺服器（5）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 95 | `/welcome` | 設定新成員歡迎訊息（{user} 會換成 @ 對方，{server} 換成伺服器名） | 被動事件 |
| 96 | `/autoreact` | 關鍵字自動反應：訊息包含某個詞時機器人加上表情 | 被動事件 |
| 97 | `/help` | 指令清單：依分類查看全部功能 | 選單 |
| 98 | `/settings` | 伺服器設定：經驗值開關、貨幣名稱、問答獎勵、AI 對話的個性 | — |
| 99 | `/botstats` | 機器人統計：上線時間、用量、最熱門的指令 | — |

### 🤖 AI 對話（1）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 100 | `/chat` | 跟機器人聊天：記得你說過的、認識你，還能幫你查歌、看活動、簽到、抽塔羅 | 按鈕、表單、被動事件 |
