...
Discord 伺服器機器人，**100 個娛樂與互動功能**：占卜算命、性格測驗、社交配對、小遊戲、趣味工具、經濟系統、模擬轉蛋、問答與音樂、等級社群、伺服器管理，以及接上 Claude 的 **AI 對話**（有記憶、認識你、會替你查資料與執行指令）。不只世界計畫，是給任何社群用的大眾向娛樂機器人；世界計畫的曲庫、卡片、卡池資料直接讀 repo 根目錄的 `data/`，跟網站同一份。

```
/塔羅 牌陣:三張牌陣 問題:這份工作該不該接
/mbti 測驗          /易經 問題:搬家好嗎          /解夢 夢境:夢到掉牙
/21點 押注:50       /踩地雷                       /wordle
/轉蛋 模式:十連      /搶答                         /抽獎 獎品:一杯手搖 分鐘:10
/聊天 內容:今天活動打到哪了？順便幫我簽到          @機器人 幫我抽一張塔羅（容器版）
```

指令、子指令、參數名稱在 Discord 上都是**中文**（`core/i18n.js` 的對照表；英文介面的使用者會看到英文 id，例如 `/tarot`）。文件與程式裡一律用英文 id 稱呼指令。
...
- **選用的 AI 層（Claude API）**：設了 `ANTHROPIC_API_KEY` 之後有兩件事：
  - **人性化的對話與互動**（`/chat`，容器版另外會回應 @機器人 或回覆它的訊息）：Claude 以這個伺服器的機器人夥伴身分回話。系統提示裡有**人設**（管理員用 `/settings ai` 選風格：活潑／溫暖／傲嬌／毒舌／正經，可改名字、加一段自訂人設）、**正在說話的這位使用者的資料**（稱呼、等級、餘額、連續簽到、推し、MBTI、戰績……所以它會說「你今天還沒簽到喔」）、以及 **100 個指令的清單**（會順手推薦）。每人每伺服器一份**對話記憶**（最近 10 個來回，6 小時沒聊就當新話題，可按「忘掉這段對話」）。模型可以用 `run_command` 工具**以使用者的身分執行指令**：查歌曲等級、活動與卡池、餘額與圖鑑、今日運勢，或替他簽到、打工、轉蛋、設提醒——工具的輸出（含卡片）會一起顯示，模型再用自己的話講重點。只准跑白名單裡的指令（不能轉帳、押注、改設定），會改資料的只在使用者明確要求時執行。
...
- **接上網站的資料**：曲庫、卡片（含 `cards-extra.js` 的技能敘述、滿等三圍、招募台詞）、卡池與活動日曆直接讀 repo 的 `data/`，網站每天更新就跟著重新部署；即時榜線與前百走**跟網站排名頁同一套來源**（HiSekai 直連 → 網站 Worker 代理 → 網站的 Haruki 備援），榜線走勢讀網站每小時的榜線快照（`data/history/`），歷史最終榜線讀 good果汁表（`data/borders-db.js`）。`/event border`、`/event top`、`/event history`、`/card`，AI 對話問「現在 T100 多少」「這張卡技能是什麼」時也會用同一組工具查。
- **固定頻道**：管理員用 `/settings channel` 指定機器人只在哪些頻道回應。其他頻道用指令會收到一句悄悄話提示，AFK 代答、自動反應、@機器人 回話也只在那些頻道；經驗值照算，管理員的設定指令不受限。
...
| 功能 | 100 個指令全部可用；但**沒有 Gateway 就收不到聊天訊息**：`/afk` 的自動回覆、`/autoreact`、`/welcome`、@機器人 就回話這四個被動功能不會動作（`/chat` 可以），`/team` 拿不到語音頻道名單；聊天經驗值改由使用指令累積 | 100 個全部 |
...
映像由 GitHub Actions 自動建好：`ghcr.io/01460421/sekai-center-bot:latest`（建置上下文是 repo 根目錄，因為要帶上 `data/` 的五個資料檔）。
...
| `COMMAND_LANG` | 否 | 預設 `zh`：註冊中文指令名（英文 id 為英文介面的顯示名）；`en` 則照英文 id 註冊 |
...
測試分四支：`registry`（恰好 100 且不超過 Discord 的 100 個全域指令上限、指令 JSON 合法）、`smoke`（每個指令與每個按鈕／選單）、`logic`（純邏輯與流程；AI 對話用假的 Claude client 測：延遲→補結果、記憶、系統提示內容、工具呼叫與白名單、額度、拒答、@機器人）、`worker`（Workers 版端對端：真的產 Ed25519 金鑰簽請求，走完驗簽 → Durable Object → 互動回應 → 持久化 → cron → 延遲回應 → 模擬 DO 休眠後重建、進行中的遊戲照樣接著玩）。
...
bot/
  ...
  src/core/
    ...
    sekai.js            世界計畫資料存取（讀 ../../data/*.js：曲庫、卡片、卡片詳情與技能、卡池）
    live.js             網站的即時與歷史資料：榜線／前百（HiSekai → 網站代理 → Haruki 備援）、榜線快照走勢、歷史最終榜線
    ai.js               選用的 AI 層（@anthropic-ai/sdk，動態載入）：narrate() 解讀、chat() 對話（含 tool loop、每日額度、拒答處理）
    i18n.js             指令的中文名對照：註冊時換成中文、收到互動換回英文 id、訊息裡的 /指令 提示換中文
    rng.js              可種子化的亂數
  src/content/          塔羅 78、易經 64、盧恩 24、星座／生肖文案、MBTI／五大／九型／心理測驗題庫、
                        解夢字典、籤詩、宜忌、幸運餅乾、Wordle／猜單字詞庫、真心話大冒險、話題、題庫…
  src/features/         100 個功能，11 個分類檔，每檔 default export 一個功能陣列（11-ai.js 是 /chat：人設、記憶、run_command 工具、@機器人 回話）
...
新功能要在 `src/core/i18n.js` 的對照表加上中文名（指令、子指令、參數），`logic` 測試會擋沒對照的功能。

**名額**：Discord 每個應用程式最多 **100 個全域斜線指令**（超過就整批註冊失敗，`registry` 測試會先擋下）。現在剛好 100 個，要加新指令得先把某個既有指令併掉——把相關功能做成同一個指令的子指令或選項不佔名額（例如歐洲人排行是 `/leaderboard by:luck`，不另開 `/luckrank`）。

## 100 個功能

Discord 上看到的是中文指令名（英文介面的使用者看到英文 id）；程式、custom_id、AI 工具與本文件一律用英文 id 稱呼。

### 🔮 占卜算命（12）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 1 | `/塔羅`（`tarot`） | 塔羅占卜：單張、三張、二選一、戀愛、五張十字牌陣 | 按鈕 |
| 2 | `/星座運勢`（`horoscope`） | 星座今日運勢（愛情／事業／財運／健康，含幸運色與數字） | 選單 |
| 3 | `/生肖運勢`（`zodiac`） | 生肖今日運勢與本命年、相合相沖 | — |
| 4 | `/易經`（`iching`） | 易經卜卦：三枚銅錢起六爻，看本卦與變卦 | — |
| 5 | `/御神籤`（`omikuji`） | 抽御神籤：大吉到大凶，附願望、戀愛、學業、工作等籤文 | 按鈕 |
| 6 | `/生命靈數`（`numerology`） | 生命靈數：由生日算出主命數、生日數、流年數 | — |
| 7 | `/盧恩`（`runes`） | 盧恩符文占卜：抽一枚或三枚古弗薩克符文 | — |
| 8 | `/今日宜忌`（`almanac`） | 今日宜忌：像老黃曆一樣，但是給現代人看的 | — |
| 9 | `/解夢`（`dream`） | 解夢：輸入夢境關鍵字或描述，找出象徵意義 | — |
| 10 | `/生日解析`（`birthchart`） | 生日全解析：星座、生肖、生命靈數、誕生花與誕生石一次看 | — |
| 11 | `/幸運餅乾`（`fortunecookie`） | 掰開今天的幸運餅乾：一句籤語加六個幸運數字 | — |
| 12 | `/占星骰`（`astrodice`） | 占星骰：擲出行星、星座、宮位三顆骰子，快速回答一個問題 | — |

### 🧠 性格測驗（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 13 | `/mbti`（`mbti`） | MBTI 十六型人格測驗（16 題），或查詢某型與配對 | 按鈕 |
| 14 | `/五大人格`（`bigfive`） | 五大人格測驗（15 題）：開放、盡責、外向、親和、神經質 | 按鈕 |
| 15 | `/戀愛類型`（`lovestyle`） | 戀愛類型測驗（8 題）：烈焰、大地、深海、微風還是全能型？ | 按鈕 |
| 16 | `/靈魂動物`（`animal`） | 靈魂動物測驗（8 題）：找出最像你的動物 | 按鈕 |
| 17 | `/色彩心理`（`color`） | 色彩心理測驗：憑直覺選一個顏色，看看現在的你 | 選單 |
| 18 | `/左右腦`（`brain`） | 左右腦測驗（10 題）：你是邏輯派還是直覺派？ | 按鈕 |
| 19 | `/九型人格`（`enneagram`） | 九型人格測驗（18 題） | 按鈕 |
| 20 | `/心理測驗`（`psytest`） | 心理測驗：一題定生死的情境小測驗，每次隨機一題 | 按鈕 |
| 21 | `/壓力指數`（`stress`） | 壓力指數測驗（10 題），看看最近的你有多緊繃 | 按鈕 |
| 22 | `/血型`（`bloodtype`） | 血型性格與血型配對 | — |

### 💞 社交互動（11）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 23 | `/配對`（`ship`） | 配對指數：算兩個人的契合度（結果固定不會變） | — |
| 24 | `/星座配對`（`zodiacmatch`） | 星座配對：兩個星座的愛情與友情相容度 | — |
| 25 | `/姓名配對`（`namematch`） | 姓名配對：輸入兩個名字算緣分 | — |
| 26 | `/結婚`（`marry`） | 結婚系統：求婚、接受、離婚、看結婚狀態 | 按鈕 |
| 27 | `/個人檔案`（`profile`） | 個人檔案：等級、水晶、稱號、推し、測驗結果、成就 | — |
| 28 | `/互動`（`interact`） | 互動動作：抱抱、摸頭、戳戳、擊掌、加油、敲頭、餵食、揮手 | — |
| 29 | `/聲望`（`rep`） | 給某人一點聲望（每人每天對同一個人一次） | — |
| 30 | `/生日`（`birthday`） | 登記生日、看今日壽星與即將到來的生日 | — |
| 31 | `/離開`（`afk`） | 設定 AFK 狀態；有人 @ 你時機器人會代為說明，你一發言就自動解除 | 被動事件 |
| 32 | `/匿名留言`（`confess`） | 匿名留言／告白：機器人代為發到這個頻道，不會記錄是誰 | — |
| 33 | `/稱號`（`title`） | 選擇顯示在個人檔案的稱號（從商店購買或成就取得） | 選單 |

### 🎮 小遊戲（12）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 34 | `/猜數字`（`guess`） | 猜數字：機器人想一個數字，你來猜（會提示大小） | — |
| 35 | `/剪刀石頭布`（`rps`） | 剪刀石頭布：跟機器人或指定的人對戰 | 按鈕 |
| 36 | `/井字遊戲`（`tictactoe`） | 井字遊戲：跟機器人或指定的人下 | 按鈕 |
| 37 | `/四子棋`（`connect4`） | 四子棋：跟機器人或指定的人對戰 | 按鈕 |
| 38 | `/21點`（`blackjack`） | 21 點：要牌、停牌、加倍，可以押水晶 | 按鈕 |
| 39 | `/拉霸`（`slots`） | 拉霸機：押注轉一把 | 按鈕 |
| 40 | `/比大小`（`hilo`） | 比大小：連續猜對彩池越滾越大，隨時可以收手 | 按鈕 |
| 41 | `/踩地雷`（`minesweeper`） | 踩地雷：5×5、4 顆雷，第一步保證安全 | 按鈕 |
| 42 | `/翻牌記憶`（`memory`） | 翻牌記憶：4×4 找出 8 組配對，越少步越多獎勵 | 按鈕 |
| 43 | `/猜單字`（`hangman`） | 猜單字（音樂與世界計畫主題的英文字） | 按鈕、表單 |
| 44 | `/wordle`（`wordle`） | Wordle：六次機會猜出五字母英文單字 | 按鈕、表單 |
| 45 | `/反應速度`（`reaction`） | 反應速度測試：按鈕變綠的瞬間按下去，比誰快 | 按鈕 |

### 🎉 趣味（12）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 46 | `/八號球`（`eightball`） | 神奇八號球：問一個是非題 | — |
| 47 | `/幫我選`（`choose`） | 幫我選：用逗號、頓號或空白分開選項 | — |
| 48 | `/擲骰`（`dice`） | 擲骰子：支援 2d6+3 這種寫法 | — |
| 49 | `/硬幣`（`coin`） | 擲硬幣（可一次擲多枚） | — |
| 50 | `/投票`（`poll`） | 發起投票（最多 5 個選項，用 \| 分開） | 按鈕 |
| 51 | `/你寧願`（`wyr`） | 你寧願：二選一，大家一起投 | 按鈕 |
| 52 | `/真心話大冒險`（`truthdare`） | 真心話大冒險 | 按鈕 |
| 53 | `/評分`（`rate`） | 幫任何東西打分數（0～10，結果固定） | — |
| 54 | `/話題`（`topic`） | 沒話聊？抽一個聊天話題 | 按鈕 |
| 55 | `/隨機點名`（`someone`） | 隨機點一個伺服器成員 | — |
| 56 | `/文字接龍`（`wordchain`） | 文字接龍：下一個詞的第一個字要接上一個詞的最後一個字 | — |
| 57 | `/故事接龍`（`story`） | 故事接龍：大家一人一句寫故事 | — |

### 💎 經濟（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 58 | `/簽到`（`daily`） | 每日簽到領水晶，連續簽到獎勵更多 | — |
| 59 | `/餘額`（`balance`） | 查看水晶餘額 | — |
| 60 | `/轉帳`（`pay`） | 轉水晶給別人 | — |
| 61 | `/打工`（`work`） | 打工賺水晶（每小時一次） | — |
| 62 | `/商店`（`shop`） | 商店：稱號、道具、禮物 | 選單 |
| 63 | `/背包`（`inventory`） | 背包：查看、使用道具，或把禮物送人 | 表單 |
| 64 | `/富豪榜`（`richlist`） | 本伺服器富豪榜（錢包＋銀行） | — |
| 65 | `/押注`（`bet`） | 押注猜硬幣：猜對翻倍 | — |
| 66 | `/樂透`（`lottery`） | 樂透：買彩券進彩池，開獎抽一位幸運兒 | — |
| 67 | `/銀行`（`bank`） | 銀行：存款每天簽到時領 1% 利息（上限 500） | — |

### 🎰 轉蛋收藏（7）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 68 | `/轉蛋`（`gacha`） | 模擬轉蛋：單抽、十連、百連（4★ 3%、3★ 8.5%，十連保底 3★） | 按鈕 |
| 69 | `/圖鑑`（`collection`） | 我的圖鑑：擁有的卡片統計，可依角色查看 | — |
| 70 | `/抽卡統計`（`gachastats`） | 抽卡統計：4★ 率、期望值、天井進度（距上次 4★、300 抽交換）、最愛角色 | — |
| 71 | `/卡片`（`card`） | 卡片查詢：稀有度、屬性、滿等綜合力、技能敘述、招募台詞（支援自動完成） | 自動完成 |
| 72 | `/交換`（`trade`） | 把你的重複卡送給別人（對方要按接受） | 按鈕 |
| 73 | `/願望單`（`wishlist`） | 願望單與推し：設定最想抽到的角色，抽到 4★ 時會特別提示 | 自動完成 |
| 74 | `/卡池情報`（`gachalist`） | 台服卡池情報：進行中與兩週內的卡池 | — |

### 🎵 問答與音樂（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 75 | `/問答`（`quiz`） | 綜合問答：世界計畫題或常識題，答對得水晶 | 按鈕 |
| 76 | `/猜歌`（`songquiz`） | 猜歌：從 BPM、長度、難度等提示猜出是哪一首 | 按鈕 |
| 77 | `/猜角色`（`charaquiz`） | 猜角色：從生日、團體、暱稱猜出是誰 | 按鈕 |
| 78 | `/搶答`（`speedquiz`） | 搶答：全頻道一起搶，最先答對的人得分 | 按鈕 |
| 79 | `/問答排行`（`quizrank`） | 問答排行榜（本伺服器） | — |
| 80 | `/歌曲`（`song`） | 歌曲查詢：難度、物量、BPM、長度 | 自動完成 |
| 81 | `/隨機選曲`（`randomsong`） | 隨機選一首歌（可指定難度與等級範圍） | 按鈕 |
| 82 | `/歌單`（`setlist`） | 隨機歌單：協力前抽 N 首歌 | — |
| 83 | `/歌曲對決`（`songbattle`） | 歌曲對決：兩首歌，大家投票選出比較喜歡的 | 按鈕 |
| 84 | `/活動`（`event`） | 台服活動：日曆、即時榜線與前百（同網站排名頁）、歷史榜線 | — |

### 🏆 等級與社群（10）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 85 | `/等級`（`rank`） | 我的等級與經驗（聊天就會累積） | 被動事件 |
| 86 | `/排行榜`（`leaderboard`） | 排行榜（本伺服器）：等級、發言、聲望、遊戲勝場、本週發言、歐洲人（4★ 率） | — |
| 87 | `/成就`（`achievements`） | 成就一覽：已解鎖與未解鎖 | — |
| 88 | `/每日任務`（`quest`） | 每日任務：完成後領取獎勵 | 按鈕 |
| 89 | `/簽到紀錄`（`streak`） | 連續簽到與活躍統計 | — |
| 90 | `/抽獎`（`raffle`） | 抽獎：大家按按鈕參加，發起人或時間到自動開獎 | 按鈕、排程 |
| 91 | `/分組`（`team`） | 隨機分組：把名單（或你所在語音頻道的人）分成 N 組 | — |
| 92 | `/提醒`（`remind`） | 設定提醒：幾分鐘後在這個頻道 @ 你 | 排程 |
| 93 | `/倒數`（`countdown`） | 倒數計時：顯示動態倒數，時間到會通知 | 排程 |
| 94 | `/計數器`（`counter`） | 共用計數器：例如記今天打了幾場協力，大家都能按 +1 | 按鈕 |

### ⚙️ 伺服器（5）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 95 | `/歡迎訊息`（`welcome`） | 設定新成員歡迎訊息（{user} 會換成 @ 對方，{server} 換成伺服器名） | 被動事件 |
| 96 | `/自動反應`（`autoreact`） | 關鍵字自動反應：訊息包含某個詞時機器人加上表情 | 被動事件 |
| 97 | `/說明`（`help`） | 指令清單：依分類查看全部功能 | 選單 |
| 98 | `/設定`（`settings`） | 伺服器設定：經驗值開關、貨幣名稱、問答獎勵、AI 對話的個性、固定頻道 | — |
| 99 | `/機器人統計`（`botstats`） | 機器人統計：上線時間、用量、最熱門的指令 | — |

### 🤖 AI 對話（1）

| # | 指令 | 說明 | 互動 |
|---|---|---|---|
| 100 | `/聊天`（`chat`） | 跟機器人聊天：記得你說過的、認識你，還能幫你查歌、看活動、簽到、抽塔羅 | 按鈕、表單、被動事件 |
