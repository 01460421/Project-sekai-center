class Component extends DCLogic {
  ...
  CHANGELOG = [
    ...
    { date: '歌曲', title: '歌曲清單（列表／方格）', desc: '完整樂曲資料庫，可切列表或封面方格檢視；支援搜尋、依團體篩選、依難度或加入時間排序，難度標籤 EASY→APPEND 完整顯示。', to: 'songs', cta: '前往歌曲清單' },
    ...
    { date: '排名', title: '活動排名', desc: 'HiSekai API 即時排名與分段榜線，每 60 秒更新；點任一排名列查看周回／場均／時速（近 1／3／24 小時）。', to: 'rank', cta: '前往活動排名' },
    ...
    { date: '分析', title: '分析中心', desc: '榜線分析(實測時速、預測終線、時間軸折線圖)、玩家分析(節奏剖析、動能、預測名次)、活躍分析(此刻在線與衝刺/降速名單)、生涯紀錄(逐期掃描過去前百,可查全部期數或指定期數)。', to: 'analysis', cta: '前往分析中心' },
    { date: '收集', title: '收集率', desc: '全 1,249 張卡片圖鑑勾選持有，即時算出總收集率與各團體、各稀有度進度；可依團體／稀有度／取得類別篩選，支援備份碼跨裝置搬移。綁定玩家 ID 後另外顯示樂曲通關／FC／AP 與角色等級。', to: 'rate', cta: '前往收集率' },
    { date: '計算', title: '計算中心', desc: 'EP 精算、效率排行、分數試算、倍率、天井、烤森、排位一站式工具。', to: 'calc', cta: '前往計算中心' },
    { date: '計算', title: '抽卡模擬', desc: '依官方 master 資料的機率模型模擬抽卡：可搜尋任一期卡池，顯示各稀有度機率與 PU 卡，支援單抽／十連（含保底★3↑），並統計已抽數、消耗石、★4 與 PU 入手、距天井。', to: 'gachasim', cta: '前往抽卡模擬' },
    { date: '計算', title: '進階計算', desc: '綜合力精算、組卡優化、分數估算與跑榜工作室（完整引擎、即時抓主資料庫）。', to: 'deckpro', cta: '前往進階計算' },
    { date: '儲值', title: '儲值商品分析', desc: '商城 930+ 項商品內容物與台幣定價、素材篩選、CP 值排行、月卡攤提，官網商店比價（同價多送 11~45%）；智慧推薦依預算或目標抽數算最省錢組合；限時商品約每小時自動更新。', to: 'shop', cta: '前往儲值分析' },
    { date: '計算', title: 'B30 產生器', desc: '勾選 FC/AP 譜面，依非官方難易度表定數（pentatonic V31）計算 Best 30 実効值，Unibot 風格版面高還原，一鍵輸出 2192×3600 PNG；可綁玩家 ID 帶入名片與 AP/FC 總數核對。', to: 'b30', cta: '前往 B30 產生器' },
  ...
  ensureEngine() {
    ...
    this._engP = new Promise((res, rej) => {
      ...
      el.src = './js/core.js?v=a48f39f71a';
      el.onload = res;
        ...
        if (!window.B30_CONSTS) {
          if (!this._b30Loading) {
            this._b30Loading = new Promise(res => {
              const s = document.createElement('script');
              s.src = 'data/b30-consts.js?v=568a80ce6e';
              s.onload = () => res();
